...
public class Tile extends PixelMapHolder {
    ...
    public Size calculateOffset(TrailPoint loc){
        //获取位图尺寸
        Size imageSize = getPixelMap().getImageInfo().size;
        //获取当前坐标所处瓦片位置
        int tile_x = getTileX(loc.lon, z);
        int tile_y = getTileY(loc.lat, z);
        //计算瓦片经度范围
        ...
        //计算Tile内偏移量
        int offset_x = -(int)((loc.lon - long_from) / (long_to - long_from) * (imageSize.width));
        int offset_y = -(int)((loc.lat - lat_from) / (lat_to - lat_from) * (imageSize.height));
    ...
    //从像素偏移量计算经纬度偏移量
    static public Point calculateOffset(int image_size, int zoom, TrailPoint loc, double offset_x, double offset_y){
        //获取当前坐标所处瓦片位置
        int tile_x = getTileX(loc.lon, zoom);
        int tile_y = getTileY(loc.lat, zoom);
        //计算瓦片经度范围
    ...
    //从longitude和latitude指定的位置计算当前瓦片显示位置的偏移量
    static public Size calculateOffset(int image_size, int zoom, TrailPoint from, TrailPoint to){
        //获取当前坐标所处瓦片位置
        int from_x = getTileX(from.lon, zoom);
        int from_y = getTileY(from.lat, zoom);
        //计算瓦片经度范围
        ...
        //计算Tile内偏移量
        double from_offset_x = ((from.lon - from_long_left) / (from_long_right - from_long_left) * image_size);
        double from_offset_y = ((from.lat - from_lat_top) / (from_lat_bottom - from_lat_top) * image_size);

        //获取当前坐标所处瓦片位置
        int to_x = getTileX(to.lon, zoom);
        int to_y = getTileY(to.lat, zoom);
        //计算瓦片经度范围
        ...
        //计算Tile内偏移量
        double to_offset_x = ((to.lon - to_long_left) / (to_long_right - to_long_left) * image_size);
        double to_offset_y = ((to.lat - to_lat_top) / (to_lat_bottom - to_lat_top) * image_size);
